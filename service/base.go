...
func RawCh()chan []byte{
	return connserver.RecvCh()
}
func ConnServerListenAndCollect(){connserver.ListenAndCollect()}
...
func PhysicalNodeChAndListenConnServer(rawch chan []byte)chan physicalnode.PhysicalNode{
	...
	go func(){
		for raw := range rawch{
			physicalNode :=protocolPreparePhysicalNode_YunHuan20200924(raw)
