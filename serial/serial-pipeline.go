...
func NewPipelineSerialConn(portname string, portbaud int, readtimeout int, needcrc bool)(string, *PipelineSerialConn){
    c := serial.Config{Name: portname, Baud: portbaud, ReadTimeout: readtimeout}
...
func (p *PipelineSerialConn)GenerateRecvCh() chan([]byte){
    ch := make(chan []byte)
    tempBuf := make([]byte, 4096)
    portName :=[]byte(p.Conn.Config.Name)
    tag :=[]byte("serial")
	go func (){
        for{    
            time.Sleep(500*time.MilliSecond)  /*阀门*/	
